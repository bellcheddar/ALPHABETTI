 ...
 *
 * Taken unchanged from ButtFold's `static/js/StageCamera.js`, which was itself ported from
 * PhoneFold's `PhoneFoldKit/Sources/FoldRender/StageCamera.swift`, commit
 * 6f44c8a1ac7684da93668a580b29cbe9a67cfc5e. Copied rather than adapted, deliberately: it is
 * the house answer to a bug all three apps had, and a fourth variant of it would be a
 * fourth thing to get wrong.
 *
 * ALPHABETTI arrived at it the long way. It used three.js OrbitControls, whose vertical
 * drag maps a full canvas height to 2*PI of polar rotation while clamping polar angle to
 * [0, PI]: half a viewport of vertical drag therefore exhausts the entire permitted range
 * and the camera pins at the pole. Measured before the change, the polar angle stopped
 * moving at 59 % of a vertical drag. Raising the sensitivity only moved that to 91 %,
 * because a spherical-coordinate camera HAS a pole and the clamp exists to protect its up
 * vector. Rotating the subject instead removes the pole rather than rationing it.
 *
 * Kept free of three.js and of DOM events for the
 * same reason the Swift is kept free of SwiftUI: the whole interaction model is then
 * testable without a browser, and the renderer only has to translate pointer events into
 * calls. The quaternion maths below is written out rather than imported for that reason -
 * it is sixteen lines, and it means the camera tests need nothing.
 *
...
export class StageCamera {
  /**
   * @param defaultDistance the distance at which the structure is framed. ALPHABETTI works
   *   in Angstroms, so the absolute number comes from the renderer's aspect fit and only
   *   the RATIOS are taken from here: in to 0.53 of the framed distance and out to 4x it,
   *   which is PhoneFold's 0.8 and 6.0 against its own 1.5.
   */
  constructor(defaultDistance = 3200) {
     ...
     * looking at it. Long enough to read as deliberate. ALPHABETTI's first version never
