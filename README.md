...
- **Flat cartoon rendering** on a button (or `M`) in every mode: toon shading, no specular highlights, bloom off. Better for a screenshot, and for reading a dense logo where every letter is catching the light at once.
- **The logo is 3D and interactive**, not a picture. Four arrangements: **helix** (a spiral tower, the default and the right one for a long family), **ring** (a crown you orbit or drop into), **strip** and **rows**. The ring and the helix orbit. The flat layouts do not: they sit square to the camera and are **navigated in two dimensions** — drag slides the sheet in x and y, shift-drag (or the zoom-box button) rubber-bands a region to zoom into, and a click centres and zooms on a column without rotating anything. A sheet of text is for reading, not orbiting.
- **A Publication button** puts the real WebLogo figure over the canvas, drawn by WebLogo itself with no interpretation from this app. Not the default view, because a 231-column alignment as a static image is six stacked rows of tiny letters, but one button (or `P`) away when you want the figure.
